...
func NewGDriveService() (*GDriveServiceImpl, error) {
	...
	return &GDriveServiceImpl{srv: srv}, nil
}

func (g *GDriveServiceImpl) DownloadFile(fileName string, destination string) error {
	//search for the file
	r, err := g.srv.Files.List().Q(fmt.Sprintf("name='%s'", fileName)).Do()
	if err != nil {
		return err
	}
	...
	file, err := g.srv.Files.Get(fileID).Download()
	if err != nil {
		return err
	}
